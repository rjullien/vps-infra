...
Surveillance automatique quotidienne de la pool site pendant les absences de OPERATOR (vacances, déplacements).

## Quand utiliser ce skill

- OPERATOR est absent et veut surveiller sa piscine à distance
...
curl -s -X POST -H "Authorization: Bearer ${HA_TOKEN}" -H "Content-Type: application/json" \
  ...
  "https://ha.example.com/api/services/button/press"
```
...
sleep 10 && curl -s -H "Authorization: Bearer ${HA_TOKEN}" \
  ...
  "https://ha.example.com/api/camera_proxy/camera.chat_snapshot"
```
...
curl -s -H "Authorization: Bearer ${HA_TOKEN}" \
  "https://ha.example.com/api/states/input_number.derniere_temperature_piscine"
```
...
**⚠️ Activer/désactiver le cron selon les vacances de OPERATOR :**
...
- **🚨 HA = PROD** : NE JAMAIS modifier les paramètres piscine (filtration, mode) sans autorisation EXPLICITE de OPERATOR
...
- **Angle caméra de référence** : `workspace/pool-watch-reference-angle.jpg` (mis à jour 09/04/2026 — vue large piscine + terrasse + jardin, position validée par OPERATOR)
- Si snapshot échoue → retenter à 12h, puis 14h. Si 3 échecs → alerter OPERATOR
