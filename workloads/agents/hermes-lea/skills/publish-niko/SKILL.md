---
name: publish-niko
description: Publish a document from amadeus-genai-docs to the shared repo genai-engineering-strategy (CONTACT_F Samberger). Use when asked to publish, share, or push a doc to the CONTACT_F repo, or to sync a document to genai-engineering-strategy. Handles review gate, merge if multiple files, clean internal tags, fix internal links, copy, tag source, commit both repos, and generate a message for CONTACT_F.
---

# Publish to CONTACT_F's Shared Repo

Publish a document from the working repo (`amadeus-genai-docs`) to the shared repo (`genai-engineering-strategy`) that CONTACT_F has access to.
...
**NEVER publish without OPERATOR's explicit "go".**
- Check source files for `✅ Reviewed by OPERATOR` tag
- If no review tag → STOP and ask OPERATOR to review first
- If OPERATOR says "go" in conversation → proceed, but tag the source files first
...
- Target = 1 topic = 1 file (clean for CONTACT_F to read)

### Step 1b — Clean for publication 🧹 MANDATORY
Before copying to target, clean the merged file:
- **Remove internal management tags:** Strip all lines containing `📤 Publié dans repo`, `✅ Reviewed by OPERATOR`, `✅ Updated`, `Status: Proposal`, or any internal tracking metadata that CONTACT_F shouldn't see
...
```
> **📤 Publié dans repo partagé avec CONTACT_F** (`genai-engineering-strategy/<dest-path>`)
> **✅ Reviewed by OPERATOR** (YYYY-MM-DD)
```
...
### Step 5 — Generate message for CONTACT_F
Compose a ready-to-copy message for OPERATOR to send to CONTACT_F:
